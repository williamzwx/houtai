...
public class SysRoleServiceImpl implements SysRoleService{
	...
	@Override
	public int saveObject(SysRole entity, Integer[] menuId) {
		//1.参数校验
		if(entity==null)
		...
		//3.保存角色菜单信息
		sysRoleMenuDao.insertObject(entity.getId(), menuId);
		//4.返回结果
		return rows;
	}
	@Override
	public SysRoleMenuVo findObjectById(Integer id) {
		//1.参数校验
		if(id==null||id<1)
			throw new IllegalArgumentException("当前角色不正确");
		//2.执行查询
		//3.返回查询结果
	...
	public int updateObject(SysRole entity, Integer[] menuIds) {
		...
		if(StringUtils.isEmpty(entity.getName()))
			throw new ServiceException("更新角色名不能为空");
	...
	public void deleteObject(Integer id) {
		//1.参数校验
		if(id==null||id<1)
			throw new ServiceException("当前角色信息有误");
